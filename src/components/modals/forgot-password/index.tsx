...
export default function ForgotPassword() {
    ...
    const choiceAPI = (data: any) => {
        API.sendPhoneMailForgotPassword(data, toggle, dispatch)
            .then(res => {
    ...
    const verify = (data: any) => {
        API.sendVerifyCodeForgot({ code: data.code }, dispatch)
            .then(res => {
    ...
    const resetPassword = (data: any) => {
        if (data.password === data.forgot_password) {
            API.reset_password(data, dispatch)
        } else {
            alert('error')
        }
    }
            ...
            >
                    ...
                    <ModalContent dividers>
                        ...
                        <Box sx={{
                            ...
                            <Form onSubmit={handleSubmit(resetPassword)}>
                                ...
                                <Input label="Подтверждение пароля" type="password" {...register('forgot_password')} />
                                <MyButton style={{ marginTop: 10 }} fullWidth>Продолжить</MyButton>
                            </Form>
                        </Box>
                    </ModalContent>
                ...
                {firstForm &&
                    <ModalContent dividers>
                        ...
                        <ToggleButton sx={{ mt: 2 }} toggleState={toggle} setToggleState={setToggle} />
                        ...
                        <Box sx={{
                            ...
                            <Form onSubmit={handleSubmit(choiceAPI)}>
                                ...
                                <MyButton style={{ marginTop: 10 }} fullWidth>Восстановить пароль</MyButton>
                            </Form>
                        </Box>
                    </ModalContent>
                }
