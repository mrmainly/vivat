...
const Contacts = () => {
    ...
    return (
        <Main>
            ...
            <MyText variant="body1" sx={{ mt: 1.6 }}>+7 (914) 123-12-12</MyText>
            {data.map((item, index) => (
                <Box key={index} sx={{ mt: 4 }}>
                    ...
                    {item.columns.map((item, index) => (
                        <Box key={index} sx={{ width: '100%', display: 'flex', justifyContent: 'space-between', mt: 1.6 }}>
                            <MyText variant="body2">{item.label}</MyText>
                            <MyText variant="body2" sx={{ color: ThemeMain.palette.primary.main }}>{item.value}</MyText>
                        </Box>
                    ))}
                </Box>
            ))}
